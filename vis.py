...
def plot_dist(desc, *paths_list, ncols=4, filename=None, titles=None, main_title=None, figsize=None,
              show_values=False, show_plot=True, symbols_in_color=True, symbol_size=180, dpi=300, force_colorbar=False):
    ...
    for axi, paths, title in zip(axes, paths_list, titles):
        ...
        if type(paths) == dict:
            ...
            if not isinstance(paths, list):
                paths = [paths]
            for path in paths:
                draw_paths(desc, axi, path, title, show_values,
                           symbols_in_color, symbol_size, force_colorbar=force_colorbar)
...
def draw_paths(desc, axi, paths, title=None, show_values=False, symbols_in_color=True, symbol_size=120, is_dist=False, force_colorbar=False):
    if paths is None:
        return
    ...
    if paths.shape in [desc.shape, (nsta,)] and not show_whole_maze:
        if is_dist:
            ...
            colorer = MplColorHelper(
                'RdBu', start_val=paths.min(), stop_val=paths.max())
            paths = paths.reshape(desc.shape)

            painted_path = colorer.paint_map(desc, paths)
            img = axi.imshow(painted_path)
            if force_colorbar:
                plt.colorbar(colorer.scalarMap, ax=axi)
...
def plot_kld(desc, pi_guess, pi_optimal, filename=None):
    ...
    plot_dist(desc, kld, main_title='KL Divergence Between Policies',
              force_colorbar=True, filename=filename)
...
def plot_errors(warmstart_errs, from_scratch_errs, filename=None):
    ...
    try:
        ...
        plt.fill_between(np.arange(len(mean)), mean - std,
                         mean + std, alpha=0.2)  # , color='r')
